...
- Git tags and App Store versions don't map one-to-one. `v1.2.0`, `v1.2.1` and
  `v1.2.2` can all carry `MARKETING_VERSION = 1.2.0`; the tag marks a build,
  the marketing version marks a submission.
- Before tagging, check whether `CURRENT_PROJECT_VERSION` needs bumping. App
  ...
  an earlier tag's Xcode Cloud build was archived there, the next tag needs a
  new build number even for the same marketing version.
